...
const CTASection = ({ data, homeCTAData }) => {
  ...
  <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 max-sm:py-16">
    <div className="grid lg:grid-cols-2 gap-16 max-sm:gap-10 items-center">
        ...
        <div className="space-y-4">
          {reasons.map((reason, index) => (
            <motion.div
              ...
              className="group flex  items-center max-sm:items-center gap-4 max-sm:gap-3 p-4 max-sm:p-3 bg-white/10 backdrop-blur-md rounded-xl border border-white/20 shadow-[0_4px_20px_rgba(0,0,0,0.2)] hover:bg-white/15 hover:border-accent/40 hover:shadow-[0_8px_32px_rgba(212,165,116,0.2)] transition-all duration-300 cursor-pointer"
            >
              <span className="text-2xl shrink-0">
                {reason.icon}
              </span>

              <p className="text-gray-200 max-sm:text-sm group-hover:text-white transition-colors">
                {reason.text}
              </p>
            </motion.div>
          ))}
      ...
      <motion.div
        ...
        className="relative"
      >
        <div className="bg-white rounded-3xl p-10 max-sm:p-6 shadow-[0_25px_60px_rgba(0,0,0,0.25)] border border-gray-100 relative overflow-hidden">
          ...
          <div className="mt-8 pt-8 border-t border-gray-100">
            <div className="flex items-center max-[383px]:flex-col gap-4">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="w-10 h-10 bg-gradient-to-br from-accent to-primary-light rounded-full border-2 border-white"
                  />
                ))}
              </div>

              <div className="max-[383px]:text-center">
